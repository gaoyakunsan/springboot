   ...
   // misc
      ...
      if (error) {
         ...
         var el = common.find('.fp-message', root)[0],
          ...
          var tmpSrc = video.src ? (video.src.length > 80 ? '' : video.src) : '';
          var tmpUrl = video.url ? (video.url.length > 80 ? '' : video.url) : '';
              common.find('h2', el)[0].innerHTML = (api.engine && api.engine.engineName || 'html5') + ": " + error.message;
         common.find('p', el)[0].innerHTML = error.url || tmpUrl || tmpSrc || conf.errorUrls[error.code];
      ...
      var api = {
         ...
         seek: function(time, callback) {
            if (api.ready && !api.live) {
               ...
               if (typeof time == "boolean") {
                  var delta = api.conf.currentSeekDelta
                      ? api.conf.currentSeekDelta
                      : api.video.duration * 0.1;
         ...
         */
         seekTo: function(position, fn) {
          ...
          seekDelta: function(delta, fn) {
            var time = delta === undefined ? lastSeekPosition : api.video.duration + delta;
            return api.seek(time, fn);
         },
...
flowplayer(function (e, o) {
    ...
    var f = function (e, a) {
        ...
        n.innerHTML = e,
            t.find(".fp-controls", o)[0].appendChild(n)
    };
    if (s && "function" == typeof key_check && key_check(s, p)) {
        if (l.logo) {
            var c = t.find(".fp-player", o)[0],
                h = i("a", {
                    className : "fp-logo"
                });
            d && (h.href = d),
            l.embed && l.embed.popup && (h.target = "_blank");
            var y = i("img", {
                src : l.logo
            });
            h.appendChild(y),
        ...
        l.brand && d || l.brand && l.brand.showOnOrigin ? f(l.brand.text || l.brand, d || location.href) : t.addClass(o, "no-brand")
    } else {
        // f("flowplayer", "http://flowplayer.org");
        var h = i("a", {
        ...
        var u = i("div", {
            ...
            c = t.find(".fp-player", o)[0];
        7 !== g && (c || o).appendChild(u),
            e.on("pause resume finish unload ready", function (e, a) {
                t.removeClass(o, "no-brand");
                var n = -1;
                if (a.video.src)
                    for (var l = [["org", "flowplayer", "drive"], ["org", "flowplayer", "my"], ["org", "flowplayer", "cdn"]], i = 0; i < l.length && (n = a.video.src.indexOf("://" + l[i].reverse().join(".")), -1 === n); i++);
                if ((4 === n || 5 === n) && t.addClass(o, "no-brand"), /pause|resume/.test(e.type) && "flash" != a.engine.engineName && 4 != n && 5 != n) {
                    var d = {
                        display : "block",
                        position : "absolute",
                        left : "16px",
                    ...
                    for (var p in d)
                        d.hasOwnProperty(p) && (h.style[p] = d[p]);
                    a.load.ed = r(h) && (7 === g || u.parentNode == o || u.parentNode == c) && !t.hasClass(o, "no-brand"),
                    a.load.ed || a.pause()
                } else
